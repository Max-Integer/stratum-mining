...
def setup(on_startup):
    ...
    *before* you call setup() in the launcher script.'''
    ...
    log.info('Connected to the Master CoinD!')
    try:
       if settings.CONFIG_VERSION != 0.2:
          log.exception("Config File Out OF Date")
          reactor.stop()
    except:
	  pass
    log.info("Connecting to mm coind...")
    while True:
        try:
        ...
        try:
            result = (yield mm_rpc.getauxblock())
            if isinstance(result, dict):
		log.error("MM Coind does not accept merged mining!")
		reactor.stop()
		break
        except ConnectionRefusedError, e:
            log.error("Connection refused while trying to connect to the coind (are your COIND_* settings correct?)")
